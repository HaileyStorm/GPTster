...
class DataLoaderLite:
    def __init__(self, B, T, process_rank, num_processes, split,
        ...
        data_root = "./data/pop3_data_offset44khz_shuffled"
    ...
    def skip_batches(self, num_batches):
        ...
        for _ in range(num_batches):
            self.next_batch(skip=True)

    def next_batch(self, skip=False):
        ...
        # remove the leading separator (1: and 2: instead of 0: and 1:)
        x = torch.stack([self.get_block(i)[1:-1] for i in process_blocks])
        y = torch.stack([self.get_block(i)[2:] for i in process_blocks])
